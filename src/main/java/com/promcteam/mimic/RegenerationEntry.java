...
public class RegenerationEntry implements ConfigurationSerializable {
    ...
    public RegenerationEntry(Map<?, ?> tempmap) {
        ...
        this.old = true;
        Mimic.getInstance().debug("RegenerationEntry %s, time: " + (new Date(this.regenTime.longValue())));
    }
    ...
    public void regenerate() {
        Optional<Map.Entry<BlockType, DataEntry>> entry = Mimic.getInstance().getData().match(this.blockType);
        ...
        Mimic.getInstance().getData().getRegenerationEntries().remove(this);
        ...
        if (dataEntry.isAgeable() && this.location.getBlock().getBlockData() instanceof Ageable) {
            Mimic.getInstance().debug("Setting block's age to " + dataEntry.getAge());
        ...
        Mimic.getInstance().getData().getTasks().remove(getLocation());
        Mimic.getInstance().debug("Regenerated at: " + this.location);
    }
}