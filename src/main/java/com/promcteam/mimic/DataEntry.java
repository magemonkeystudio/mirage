...
import com.promcteam.mimic.requirements.Requirement;
import com.promcteam.mimic.rewards.AmountReward;
import com.promcteam.mimic.rewards.Reward;
import com.promcteam.mimic.tools.ToolType;
...
public class DataEntry {
    ...
    public DataEntry(Map<String, Object> map) {
            ...
            if (object instanceof List) {
                ...
                for (Object obj : list) {
                    try {
                        ...
                        Mimic.getInstance().getLogger().warning("Ignoring invalid reward/cost \"" + obj + "\":");
                        e.printStackTrace();
                    }
                }
            } else if (object instanceof String) {
                try {
                    ...
                    Mimic.getInstance().getLogger().warning("Ignoring invalid reward/cost \"" + object + "\":");
                    e.printStackTrace();
                }
            }
        ...
        if (object instanceof Map) {
            ...
            for (Map.Entry<?, ?> entry : chanceMap.entrySet()) {
                ...
                if (weight instanceof Number) {
                    ...
                    if (chance == 0) {
                        Mimic.getInstance().getLogger().warning("Ignoring 'chance' key with 0 weight: " + key);
                        continue;
                    }
                } else {
                    Mimic.getInstance().getLogger().warning("Ignoring invalid 'chance' weight: " + weight);
                    continue;
                }
        ...
        Mimic.getInstance().debug("Created DataEntry for materials:");
        for (BlockType blockType : this.materials) {
            Mimic.getInstance().debug("- " + blockType.getPrefix());
        }
    }
    ...
    public boolean meetsRequirements(Player player) {
        for (Requirement requirement : this.requirements) {
            if (!requirement.meets(player)) {
                Mimic.getInstance().debug("Requirement not met: " + requirement);
                return false;
            }
        }
        return true;
    ...
    public AmountReward applyCostsAndRewards(Player player, boolean apply) {
        Lang lang = Mimic.getInstance().getLang();
    ...
    public static class Command {
        ...
        public void execute(Player player) {
            ...
            Bukkit.getScheduler().runTaskLater(Mimic.getInstance(), () -> {
                Object sender;
