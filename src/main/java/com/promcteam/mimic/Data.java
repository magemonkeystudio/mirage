...
public class Data {
    ...
    public void load(ConfigurationSection section) {
        ...
        section.getMapList("entries").stream()
                .map(map -> {
                    try {
                        ...
                        Mimic.getInstance().getLogger().info("Invalid entry: " + e.getMessage());
                        e.printStackTrace();
                        return null;
                    }
                ...
                .forEach(dataEntry -> {
                    for (BlockType blockType : dataEntry.getMaterials()) {
                        ...
                        Mimic.getInstance().debug("Registered entry for block type " + blockType + "'");
                        if (existing != null) {
                            Mimic.getInstance()
        ...
        Mimic.getInstance().getLogger().info("Loaded " + this.entries.size() + " entries.");
    }
    ...
    public Optional<Map.Entry<BlockType, DataEntry>> match(Block block) {
        for (Map.Entry<BlockType, DataEntry> entry : this.entries.entrySet()) {
            ...
            Mimic.getInstance().debug("Comparing broken " + block.getType() + " with " + blockType);
            if (!blockType.isInstance(block)) {
                Mimic.getInstance().debug("Not a match.");
                continue;
            }

            Mimic.getInstance().debug("We have a match.");
            return Optional.of(entry);
        }

        Mimic.getInstance().debug("No matches found.");
    ...
    public Optional<Map.Entry<BlockType, DataEntry>> match(BlockType blockType) {
        for (Map.Entry<BlockType, DataEntry> entry : this.entries.entrySet()) {
            ...
            Mimic.getInstance().debug("Comparing broken " + blockType1 + " with " + blockType);
    ...
    public void saveRegenerationEntries() throws IOException {
        ...
        Mimic.getInstance().getLogger().info("Saved " + list.size() + " entries.");
    }
    ...
    public void startRegenerationTask(RegenerationEntry entry) {
        ...
        this.tasks.put(entry.getLocation(),
                Bukkit.getScheduler().runTaskLater(Mimic.getInstance(), entry::regenerate, (entry
    ...
    public void startRegenerationWatchdog() {
        ...
        this.watchdog = Bukkit.getScheduler().runTaskTimerAsynchronously(Mimic.getInstance(),
                () -> this.regenerationEntries.stream().filter(RegenerationEntry::isOld)
                        .peek(e -> Mimic.getInstance()
                        ...
                        .peek(e -> Mimic.getInstance().getLogger().info("Watchdog: " + e.getLocation()))
